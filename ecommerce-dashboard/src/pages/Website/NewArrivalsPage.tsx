...
// TODO: this is a manual placement list feeding /collections/new-arrivals and
// the homepage new-arrivals row — order here maps 1:1 to display order.
// Note "Best Sellers" has no admin screen at all — it's calculated from
// delivered-order counts per the storefront business logic, not curated here.
const INITIAL_NEW_ARRIVAL_IDS = ["prod-dates-ajwa", "prod-oil-mustard"];

const NewArrivalsPage = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [newArrivalIds, setNewArrivalIds] = useState<string[]>(INITIAL_NEW_ARRIVAL_IDS);
  const [pickerOpen, setPickerOpen] = useState(false);

  const rows = useMemo(
    () => newArrivalIds.map((id, index) => ({ product: DUMMY_PRODUCTS.find((p) => p._id === id) as IProduct, order: index + 1 })).filter((r) => r.product),
    [newArrivalIds]
  );

  const availableProducts = DUMMY_PRODUCTS.filter((p) => !newArrivalIds.includes(p._id));

  const handleAdd = (ids: string[]) => {
    setNewArrivalIds((prev) => [...prev, ...ids]);
    toast.success(`${ids.length} product${ids.length === 1 ? "" : "s"} added to New Arrivals`);
  };
  ...
  return (
    <PageWraper
      title="New Arrivals"
      description="Manually curated products shown in the homepage new-arrivals row and /collections/new-arrivals."
    ...
    >
      ...
      <ProductPickerModal
        ...
        description="Select one or more products to show in the new-arrivals row."
